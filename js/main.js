...
var aniTime = 1020;
var cssAnimatedBag = "animatedBag";
var cssMovedBag = "movedBag";

var bagIcons = ['img/bag_black.png', 'img/bag_black2.png', 'img/bag_brown.png', 'img/bag_brown2.png', 'img/bag_light_brown.png'];
...
function parse(haystack, tag) {
	...
	// parse
	return haystack.substring(haystack.indexOf(tagBegin) + tagBegin.length, haystack.indexOf(tagEnd));
}
//*****************************************************************************
/*
...
function log(text, c, useTimestamp) {
	...
	if ((year = timestamp.getYear()) < 2000)
		year += 1900;
	if ((month = timestamp.getMonth() + 1) < 10)
		month = "0" + month;
	if ((day = timestamp.getDate()) < 10)
		day = "0" + day;
	if ((hours = timestamp.getHours()) < 10)
		hours = "0" + hours;
	if ((minutes = timestamp.getMinutes()) < 10)
		minutes = "0" + minutes;
	if ((seconds = timestamp.getSeconds()) < 10)
		seconds = "0" + seconds;
	
	// set timeformat
	var timeformat = year + "-" + month + "-" + day + " " + hours + ":" + minutes + ":"+ seconds;
	var msg = useTimestamp ? ($c.val().length != 0 ? '\n' : '') + '[' + timeformat + '] ' + text : text;
...
function retrieveBaggageList(requestUrl) {
	...
	$('#getBag').attr('src', 'img/gui/list-remove-disabled.png').attr('disabled', 'disabled');
}
//*****************************************************************************
/*
...
function setEvent(event, value, requestUrl) {
	$.get(requestUrl, {action: 'setEvent', perform: event, value: value}, function(data) {
		log(data);
	});
...
function getEvent(event, requestUrl) {
	$.get(requestUrl, {action: "getEvent", perform: event}, function(data) {
		...
		switch (event) {
			case 'autoMode':
				$('#btnAutoMode').attr('src', isEnabled ? 'img/gui/media-repeat-active.png' : 'img/gui/media-repeat.png').val(newVal);
			break;
			case 'scheduler':
				$('#btnScheduler').attr('src', isEnabled ? 'img/gui/media-play-active.png' : 'img/gui/media-play.png').val(newVal);
			break;
			case '24hLimit':
				$('#btn24hLimit').attr('src', isEnabled ? 'img/gui/24h-active.png' : 'img/gui/24h.png').val(newVal);
			break;
			default:
		}
	});
...
function putBag(id, x, y, z, w, h, ox, oy, zi) {
	...
	if ($elem.length > 0) {
		...
		$elem.animate({
			left: x2,
			top: y2
		}, aniTime, 'linear').css("z-index", zi).dequeue();
		...
		$("#bags").append("<img src=\"" + bagImg + "\" class=\"" + cssAnimatedBag + " " + cssMovedBag + "\" id=\"bag_" + id + "\" width=\"" + w2 + "\" height=\"" + h2 + "\" style=\"position: absolute; left: " + x2 + "px; top: " + y2 + "px; opacity: 0;\"/>");
		$(sel).fadeTo(300, 0.9);
	}
 ...
 */
 function flushBags() {
	...
	$("." + cssAnimatedBag).not("." + cssMovedBag).each(function() {
...
function setImageOffset() {
	var initX = 20;
	var initY = 100;
	$('#bags').html('<IMG ID="offsetImg" SRC="img/terminal.png" WIDTH="29" HEIGHT="29" STYLE="visibility: hidden; POSITION: absolute; LEFT: ' + initX + 'px; TOP: ' + initY + 'px;"/>');
...
function renderImage() {
	...
	var imgSrc = getImageURL() + '&mark=' + markedBag + '&labels=' + labels + '&use3d=' + use3d + '&useAnimations=' + useAnimations + '&rtime=' + renderTime + '&stats=' + stats + '&textColor=' + textColor + '&renderStyle=' + renderStyle;
...
function restoreUISession() {
	...
	$('#btn3d').attr('src', 'img/gui/3d' + (load('setting.use3d') ? '-active' : '') + '.png');
	$('#btnBaggageIDs').attr('src', 'img/gui/id' + (load('setting.showBagIDs') ? '-active' : '') + '.png');
	$('#btnStatistics').attr('src', 'img/gui/statistics' + (load('setting.showStats') ? '-active' : '') + '.png');
	$('#btnRenderTime').attr('src', 'img/gui/render-time' + (load('setting.showRenderTime') ? '-active' : '') + '.png');
	$('#btnAnimations').attr('src', 'img/gui/animations' + (load('setting.useAnimations') ? '-active' : '') + '.png');
	...
	if (aniIcon !== undefined) {
		$('input[name="animationIcon"][value="' + aniIcon + '"]:radio').prop('checked', true);
	}
	...
	if (storedUIElements == null || storedUIElements === undefined) {
		...
		for (key in uiElements) {
			...
			$('fieldset[data-fieldset-id="' + o.id + '"]').offset({left: o.x, top: o.y});
		}
	}
	...
	$('#checkboxShowHelpBubbles').prop('checked', load('setting.showHelpBubbles') != false);
	$('#checkboxShowQueries').prop('checked', load('setting.showQueries') != false);
...
function addBag(terminal) {
	...
	$.get(getQueryURL(), {action: 'placeBaggage', id: terminal}, function(data) {
		log(data);
		retrieveBaggageList(getQueryURL());
	});

	// animate the terminal buttons
	$('.terminalImg[value="' + terminal + '"]').animate({
		marginLeft: "-20px",
		opacity: 1
	}, 250).animate({
		marginLeft: "0px",
...
function getBag(bag, terminal) {
	...
	$.get(getQueryURL(), {action: 'retrieveBaggage', bagId: bag, dest: terminal}, function(data) {
		retrieveBaggageList(getQueryURL());
		log(data);
	});
...
$(function() {
	...
	$('#btn3d').click(function() {
		...
		$(this).attr('src', 'img/gui/3d' + (load('setting.use3d') ? '-active' : '') + '.png');
	});
	$('#btnBaggageIDs').click(function() {
		store('setting.showBagIDs', !load('setting.showBagIDs'));
		$(this).attr('src', 'img/gui/id' + (load('setting.showBagIDs') ? '-active' : '') + '.png');
	});
	$('#btnStatistics').click(function() {
		store('setting.showStats', !load('setting.showStats'));
		$(this).attr('src', 'img/gui/statistics' + (load('setting.showStats') ? '-active' : '') + '.png');
	});
	$('#btnRenderTime').click(function() {
		store('setting.showRenderTime', !load('setting.showRenderTime'));
		$(this).attr('src', 'img/gui/render-time' + (load('setting.showRenderTime') ? '-active' : '') + '.png');
	});
	$('#btnAnimations').click(function() {
		store('setting.useAnimations', !load('setting.useAnimations'));
		$(this).attr('src', 'img/gui/animations' + (load('setting.useAnimations') ? '-active' : '') + '.png');
	});
	...
	positionNextTo($('#animationIconsPopup'), $('#btnAnimations'), 'right', -3, 0);
	...
	if (load('setting.showHelpBubbles') != false) {
		$('fieldset[data-fieldset-id="bagImage"] .bubble').fadeIn(1000).delay(2000).fadeOut(2000);
	}
